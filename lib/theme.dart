import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

double defaultMargin = 30.0;

Color primaryColor = Color(0xff6C5ECF);
Color secondaryColor = Color(0xff38ABBE);
Color alertColor = Color(0xffED6363);
Color priceColor = Color(0xff2C96F1);
Color bgColor1 = Color(0xff1F1D2B);
Color bgColor2 = Color(0xff2B2937);
Color bgColor3 = Color(0xff242231);
Color bgColor4 = Color(0xff504F5E);
Color bgColor5 = Color(0xff2B2844);
Color primaryTextColor = Color(0xffE1E1E1);
Color secondaryTextColor = Color(0xff999999);
Color signTextColor = Color(0xff504F5E);
Color bottomNavColor = Color(0xfff252836);
Color innactiveNavbarColor = Color(0xff808191);

TextStyle primaryTextStyle = GoogleFonts.poppins(color: primaryTextColor);
TextStyle secondaryTextStyle = GoogleFonts.poppins(color: secondaryTextColor);
TextStyle priceTextStyle = GoogleFonts.poppins(color: priceColor);
TextStyle subtittleColor = GoogleFonts.poppins(color: signTextColor);
TextStyle pupleTexStyle = GoogleFonts.poppins(color: primaryColor);
TextStyle textColorPrimaryStyle = GoogleFonts.poppins(color: bottomNavColor);

FontWeight light = FontWeight.w300;
FontWeight regular = FontWeight.w400;
FontWeight medium = FontWeight.w500;
FontWeight semiBold = FontWeight.w600;
FontWeight bold = FontWeight.w700;
